 ...
 */
function createContragentsSheet(
  spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet
): GoogleAppsScript.Spreadsheet.Sheet {
  // Create new sheet
  ...
  sheet
    .getRange(3, 1)
    .setValue('↑ Edit the row above or add more rows with your client information');
  sheet.getRange(3, 1, 1, headers.length).merge();
  sheet.getRange(3, 1).setFontStyle('italic');
  sheet.getRange(3, 1).setFontColor('#666666');
...
function createSampleInvoiceSheet(spreadsheet: GoogleAppsScript.Spreadsheet.Spreadsheet): void {
  ...
  if (spreadsheet.getSheets().length <= 2) {
    ...
    // Add a note
    sheet
      .getRange(sampleData.length + 2, 1)
      .setValue(
        'This is a sample invoice sheet. Select the rows with data and use the "Generate Invoice" option from the "Invoice Generator" menu.'
      );
